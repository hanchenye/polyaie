...
void PlacementSimple::runOnOperation() {
    ...
    int Port_in = A * std::ceil((float)(C/C)) * std::ceil((float)(B/4.0)) + C * std::ceil((float)(A/2.0)) * std::ceil((float)(B/4.0));
    ...
    for (auto process : topFunc.getOps<ProcessOp>()) {
      ...
      if(!(process->getAttr("polyaie.col"))){
        ...
        if (auto tensorOp=accumOperand.getDefiningOp<dataflow::TensorLoadOp>()){
          ...
          tensorOp->setAttr("polyaie.col",b.getI64IntegerAttr(47));
    ...
    for (auto process : topFunc.getOps<ProcessOp>()) {
      ...
      if (auto tensorOp=accumOperand.getDefiningOp<dataflow::TensorLoadOp>()){
        ...
        auto source_L = process->getOperand(0);
        auto source_R = process->getOperand(1);
        ...
        if(!(tensorOp_SR->getAttr("polyaie.col"))){
          ...
          tensorOp_SR->setAttr("polyaie.PackID",b.getI64IntegerAttr(ID_R%4));
          ID_R++;
        }
          ...
          for (auto process_child : topFunc.getOps<ProcessOp>()) {
            ...
            if (accumOperand_child==result){
              ...
              if(!(tensorOp_R->getAttr("polyaie.col"))){
                ...
                tensorOp_R->setAttr("polyaie.PackID",b.getI64IntegerAttr(ID_R%4));
                flag=1;
                ID_R++;
                result=process_child.getResult(0);
                end_chain1=0;
    ...
    //Fourth round
    //Replace tensor_load and tensor_store with process
