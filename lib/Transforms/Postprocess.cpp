...
void Postprocess::runOnOperation() {
  ...
  for (auto &op : llvm::make_early_inc_range(mod.getBody()->getOperations())) {
    ...
    } else if (auto dma = dyn_cast<dataflow::HostDMAOp>(op)) {
      // Collect leaf tiles in the program.
      if (dma.kind() == HostDMAKind::AIEToHost)
        leafTiles.insert(dma.source().getDefiningOp<BufferOp>().getTileOp());
    ...
    } else if (isa<TokenOp, WireOp, PLIOOp, ShimMuxOp>(op)) {
