...
#define DMA_COUNT 1024
  ...
  int *mem_ptr8 =
      (int *)mmap(NULL, 4100, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0xC000);
  int *mem_ptr9 =
      (int *)mmap(NULL, 4100, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0xE000);
  int *mem_ptr10 =
  ...
  for (int idx0 = 0; idx0 < 1025; ++idx0) {
    ...
    } else if (mem_ptr8[idx0] != 64) {
      printf("External memory2[%d]=%d\n", idx0, mem_ptr8[idx0]);
      errors++;
    }
  }

  mlir_aie_check("Before release lock:", mlir_aie_read_buffer_buf33(_xaie, 0),
                 32, errors);
  mlir_aie_check("Before release lock:", mlir_aie_read_buffer_buf38(_xaie, 0),
                 1, errors);
  mlir_aie_check("Before release lock:", mlir_aie_read_buffer_buf39(_xaie, 0),
                 1, errors);
