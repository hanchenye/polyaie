...
void gemm(float arg0[64][64], float arg1[64][64], float arg2[64][64],
  ...
  for (int64_t idx0 = 0; idx0 < 32; ++idx0)
    for (int64_t idx1 = 0; idx1 < 32; ++idx1)
      buf4_ptr[bufIdx++] = arg1[idx0 + 32][idx1 + 32];
  ...
  for (int64_t idx0 = 0; idx0 < 32; ++idx0)
    for (int64_t idx1 = 0; idx1 < 32; ++idx1)
      buf5_ptr[bufIdx++] = arg1[idx0 + 32][idx1];
  ...
  for (int64_t idx0 = 0; idx0 < 32; ++idx0)
    for (int64_t idx1 = 0; idx1 < 32; ++idx1)
      buf6_ptr[bufIdx++] = arg2[idx0 + 32][idx1 + 32];
  ...
  for (int64_t idx0 = 0; idx0 < 32; ++idx0)
    for (int64_t idx1 = 0; idx1 < 32; ++idx1)
      buf7_ptr[bufIdx++] = arg2[idx0][idx1 + 32];
  ...
  while (!kernel_complete()) {
    if (mlir_aie_acquire_lock(_xaie, 10, 3, 15, 1, 0))
      results[0] = true;
    if (mlir_aie_acquire_lock(_xaie, 11, 3, 15, 1, 0))
      results[1] = true;
    if (mlir_aie_acquire_lock(_xaie, 12, 3, 15, 1, 0))
      results[2] = true;
    if (mlir_aie_acquire_lock(_xaie, 13, 3, 15, 1, 0))
      results[3] = true;
  }
  ...
  float *buf9_ptr =
      (float *)mmap(NULL, 4100, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0xE000);
  bufIdx = 0;
  for (int64_t idx0 = 0; idx0 < 1025; ++idx0)
    arg0[idx0] = buf9_ptr[bufIdx++];
