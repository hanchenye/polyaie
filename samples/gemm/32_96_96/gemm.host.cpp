...
void gemm(float arg0[32][96], float arg1[32][96], float arg2[96][96],
          unsigned iter_num = 1) {
  ...
  for (int64_t idx0 = 0; idx0 < 32; ++idx0)
    for (int64_t idx1 = 0; idx1 < 32; ++idx1)
      buf6_ptr[bufIdx++] = arg2[idx0 + 64][idx1 + 64];
  ...
  for (int64_t idx0 = 0; idx0 < 32; ++idx0)
    for (int64_t idx1 = 0; idx1 < 32; ++idx1)
      buf7_ptr[bufIdx++] = arg2[idx0 + 32][idx1 + 64];
  ...
  for (int64_t idx0 = 0; idx0 < 32; ++idx0)
    for (int64_t idx1 = 0; idx1 < 32; ++idx1)
      buf8_ptr[bufIdx++] = arg2[idx0][idx1 + 64];
  ...
  for (int64_t idx0 = 0; idx0 < 32; ++idx0)
    for (int64_t idx1 = 0; idx1 < 32; ++idx1)
      buf9_ptr[bufIdx++] = arg2[idx0 + 64][idx1 + 32];
  ...
  for (int64_t idx0 = 0; idx0 < 32; ++idx0)
    for (int64_t idx1 = 0; idx1 < 32; ++idx1)
      buf10_ptr[bufIdx++] = arg2[idx0 + 32][idx1 + 32];
  ...
  for (int64_t idx0 = 0; idx0 < 32; ++idx0)
    for (int64_t idx1 = 0; idx1 < 32; ++idx1)
      buf11_ptr[bufIdx++] = arg2[idx0][idx1 + 32];
  ...
  for (int64_t idx0 = 0; idx0 < 1025; ++idx0)
    arg0[idx0] = buf12_ptr[bufIdx++];

  float *buf13_ptr = (float *)mmap(NULL, 4100, PROT_READ | PROT_WRITE,
                                   MAP_SHARED, fd, 0x12000);
  bufIdx = 0;
  for (int64_t idx0 = 0; idx0 < 1025; ++idx0)
    arg0[idx0] = buf13_ptr[bufIdx++];

  float *buf14_ptr = (float *)mmap(NULL, 4100, PROT_READ | PROT_WRITE,
                                   MAP_SHARED, fd, 0x14000);
  bufIdx = 0;
  for (int64_t idx0 = 0; idx0 < 1025; ++idx0)
    arg0[idx0] = buf14_ptr[bufIdx++];
