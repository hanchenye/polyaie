...
void gemm(float arg0[32][64], float arg1[32][64], float arg2[64][64],
          unsigned iter_num = 1) {
  ...
  for (int64_t idx0 = 0; idx0 < 32; ++idx0)
    for (int64_t idx1 = 0; idx1 < 32; ++idx1)
      buf0_ptr[bufIdx++] = arg2[idx0 + 32][idx1 + 32];
  ...
  for (int64_t idx0 = 0; idx0 < 32; ++idx0)
    for (int64_t idx1 = 0; idx1 < 32; ++idx1)
      buf1_ptr[bufIdx++] = arg2[idx0][idx1 + 32];
  ...
  float *buf6_ptr =
      (float *)mmap(NULL, 4100, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0xA000);
  bufIdx = 1;
  for (int64_t idx0 = 0; idx0 < 1024; ++idx0)
    arg0[idx0 / 32][idx0 % 32] = buf6_ptr[bufIdx++];

  float *buf7_ptr =
      (float *)mmap(NULL, 4100, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0xB004);
  bufIdx = 1;
  for (int64_t idx0 = 0; idx0 < 1024; ++idx0)
    arg0[idx0 / 32][idx0 % 32 + 32] = buf7_ptr[bufIdx++];
