void mlir_aie_configure_cores(aie_libxaie_ctx_t *ctx) {
  XAieTile_ShimColumnReset(&(ctx->TileInst[27][0]), XAIE_RESETENABLE);
  // Reset configuration
  // Reset configuration
  // ShimDMA
  for (int i = 0x1D000; i <= 0x1D13C; i += 4)
    XAieGbl_Write32((&(ctx->TileInst[27][0]))->TileAddr + i, 0);
  ...
  for (int i = 0x3F200; i <= 0x3F37C; i += 4)
    XAieGbl_Write32((&(ctx->TileInst[27][0]))->TileAddr + i, 0);
  XAieTile_ShimColumnReset(&(ctx->TileInst[27][0]), XAIE_RESETDISABLE);
  ...
  XAieTile_CoreControl(&(ctx->TileInst[27][2]), XAIE_DISABLE, XAIE_ENABLE);
  // Reset configuration
  // Program Memory
  for (int i = 0x20000; i <= 0x23FFF; i += 4)
    XAieGbl_Write32((&(ctx->TileInst[27][2]))->TileAddr + i, 0);
  // TileDMA
  for (int i = 0x1D000; i <= 0x1D1F8; i += 4)
    XAieGbl_Write32((&(ctx->TileInst[27][2]))->TileAddr + i, 0);
  for (int i = 0x1DE00; i <= 0x1DE00; i += 4)
  ...
  for (int i = 0x3F200; i <= 0x3F3AC; i += 4)
    XAieGbl_Write32((&(ctx->TileInst[27][2]))->TileAddr + i, 0);
  for (int l = 0; l < 16; l++)
    XAieTile_LockRelease(&(ctx->TileInst[27][2]), l, 0x0, 0);
  {
    int ret = XAieGbl_LoadElf(&(ctx->TileInst[27][2]), (u8 *)"core_27_2.elf",
                              XAIE_ENABLE);
    if (ret == XAIELIB_FAILURE)
      printf("Failed to load elf for Core[%d,%d], ret is %d", 27, 2, ret);
    assert(ret != XAIELIB_FAILURE);
  }
  XAieTile_CoreControl(&(ctx->TileInst[27][3]), XAIE_DISABLE, XAIE_ENABLE);
  // Reset configuration
  // Program Memory
  for (int i = 0x20000; i <= 0x23FFF; i += 4)
    XAieGbl_Write32((&(ctx->TileInst[27][3]))->TileAddr + i, 0);
  // TileDMA
  for (int i = 0x1D000; i <= 0x1D1F8; i += 4)
    XAieGbl_Write32((&(ctx->TileInst[27][3]))->TileAddr + i, 0);
  for (int i = 0x1DE00; i <= 0x1DE00; i += 4)
  ...
  for (int i = 0x3F200; i <= 0x3F3AC; i += 4)
    XAieGbl_Write32((&(ctx->TileInst[27][3]))->TileAddr + i, 0);
  for (int l = 0; l < 16; l++)
    XAieTile_LockRelease(&(ctx->TileInst[27][3]), l, 0x0, 0);
  {
    int ret = XAieGbl_LoadElf(&(ctx->TileInst[27][3]), (u8 *)"core_27_3.elf",
                              XAIE_ENABLE);
    if (ret == XAIELIB_FAILURE)
      printf("Failed to load elf for Core[%d,%d], ret is %d", 27, 3, ret);
    assert(ret != XAIELIB_FAILURE);
  }
  XAieTile_CoreControl(&(ctx->TileInst[27][1]), XAIE_DISABLE, XAIE_ENABLE);
  // Reset configuration
  // Program Memory
  for (int i = 0x20000; i <= 0x23FFF; i += 4)
    XAieGbl_Write32((&(ctx->TileInst[27][1]))->TileAddr + i, 0);
  // TileDMA
  for (int i = 0x1D000; i <= 0x1D1F8; i += 4)
    XAieGbl_Write32((&(ctx->TileInst[27][1]))->TileAddr + i, 0);
  for (int i = 0x1DE00; i <= 0x1DE00; i += 4)
  ...
  for (int i = 0x3F200; i <= 0x3F3AC; i += 4)
    XAieGbl_Write32((&(ctx->TileInst[27][1]))->TileAddr + i, 0);
  for (int l = 0; l < 16; l++)
    XAieTile_LockRelease(&(ctx->TileInst[27][1]), l, 0x0, 0);
...
void mlir_aie_start_cores(aie_libxaie_ctx_t *ctx) {
  ...
  XAieTile_CoreControl(&(ctx->TileInst[27][2]), XAIE_ENABLE, XAIE_DISABLE);
  XAieTile_CoreControl(&(ctx->TileInst[27][3]), XAIE_ENABLE, XAIE_DISABLE);
  XAieTile_CoreControl(&(ctx->TileInst[27][1]), XAIE_ENABLE, XAIE_DISABLE);
...
void mlir_aie_configure_dmas(aie_libxaie_ctx_t *ctx) {
  ...
  XAieDma_TileChControl(&(ctx->TileDMAInst[26][3]), XAIEDMA_TILE_CHNUM_MM2S0,
                        XAIE_RESETDISABLE, XAIE_ENABLE);
  XAieDma_TileInitialize(&(ctx->TileInst[27][2]), &(ctx->TileDMAInst[27][2]));
  XAieDma_TileBdClearAll(&(ctx->TileDMAInst[27][2]));
  XAieDma_TileChResetAll(&(ctx->TileDMAInst[27][2]));
  XAieDma_TileBdSetLock(&(ctx->TileDMAInst[27][2]), /* bd */ 0,
                        XAIEDMA_TILE_BD_ADDRA, /* lockID */ 1, XAIE_ENABLE,
                        /* release */ 1, XAIE_ENABLE, /* acquire */ 0);
  XAieDma_TileBdSetAdrLenMod(&(ctx->TileDMAInst[27][2]), /* bd */ 0,
                             /* addrA */ 0x3000, /* addrB */ 0x0,
                             /* len */ 1024 * 4, /* ABMode */ XAIE_DISABLE,
                             /* FIFOMode */ XAIE_DISABLE);
  XAieDma_TileBdSetNext(&(ctx->TileDMAInst[27][2]), /* bd */ 0, /* nextbd */ 0);
  XAieDma_TileBdSetPkt(&(ctx->TileDMAInst[27][2]), /* bd */ 0, /* en */ 1,
                       /* type */ 0, /* id */ 4);
  XAieDma_TileBdWrite(&(ctx->TileDMAInst[27][2]), /* bd */ 0);
  XAieDma_TileBdSetLock(&(ctx->TileDMAInst[27][2]), /* bd */ 1,
                        XAIEDMA_TILE_BD_ADDRA, /* lockID */ 0, XAIE_ENABLE,
                        /* release */ 1, XAIE_ENABLE, /* acquire */ 0);
  XAieDma_TileBdSetAdrLenMod(&(ctx->TileDMAInst[27][2]), /* bd */ 1,
                             /* addrA */ 0x2000, /* addrB */ 0x0,
                             /* len */ 1024 * 4, /* ABMode */ XAIE_DISABLE,
                             /* FIFOMode */ XAIE_DISABLE);
  XAieDma_TileBdSetNext(&(ctx->TileDMAInst[27][2]), /* bd */ 1, /* nextbd */ 1);
  XAieDma_TileBdSetPkt(&(ctx->TileDMAInst[27][2]), /* bd */ 1, /* en */ 1,
                       /* type */ 0, /* id */ 0);
  XAieDma_TileBdWrite(&(ctx->TileDMAInst[27][2]), /* bd */ 1);
  XAieDma_TileSetStartBd((&(ctx->TileDMAInst[27][2])), XAIEDMA_TILE_CHNUM_S2MM0,
                         /* bd */ 0);
  XAieDma_TileChControl(&(ctx->TileDMAInst[27][2]), XAIEDMA_TILE_CHNUM_S2MM0,
                        XAIE_RESETDISABLE, XAIE_ENABLE);
  XAieDma_TileSetStartBd((&(ctx->TileDMAInst[27][2])), XAIEDMA_TILE_CHNUM_S2MM1,
                         /* bd */ 1);
  XAieDma_TileChControl(&(ctx->TileDMAInst[27][2]), XAIEDMA_TILE_CHNUM_S2MM1,
                        XAIE_RESETDISABLE, XAIE_ENABLE);
  XAieDma_TileInitialize(&(ctx->TileInst[27][3]), &(ctx->TileDMAInst[27][3]));
  XAieDma_TileBdClearAll(&(ctx->TileDMAInst[27][3]));
  XAieDma_TileChResetAll(&(ctx->TileDMAInst[27][3]));
  XAieDma_TileBdSetLock(&(ctx->TileDMAInst[27][3]), /* bd */ 0,
                        XAIEDMA_TILE_BD_ADDRA, /* lockID */ 0, XAIE_ENABLE,
                        /* release */ 1, XAIE_ENABLE, /* acquire */ 0);
  XAieDma_TileBdSetAdrLenMod(&(ctx->TileDMAInst[27][3]), /* bd */ 0,
                             /* addrA */ 0x3000, /* addrB */ 0x0,
                             /* len */ 1024 * 4, /* ABMode */ XAIE_DISABLE,
                             /* FIFOMode */ XAIE_DISABLE);
  XAieDma_TileBdSetNext(&(ctx->TileDMAInst[27][3]), /* bd */ 0, /* nextbd */ 0);
  XAieDma_TileBdSetPkt(&(ctx->TileDMAInst[27][3]), /* bd */ 0, /* en */ 1,
                       /* type */ 0, /* id */ 5);
  XAieDma_TileBdWrite(&(ctx->TileDMAInst[27][3]), /* bd */ 0);
  XAieDma_TileBdSetLock(&(ctx->TileDMAInst[27][3]), /* bd */ 1,
                        XAIEDMA_TILE_BD_ADDRA, /* lockID */ 3, XAIE_ENABLE,
                        /* release */ 1, XAIE_ENABLE, /* acquire */ 0);
  XAieDma_TileBdSetAdrLenMod(&(ctx->TileDMAInst[27][3]), /* bd */ 1,
                             /* addrA */ 0x2000, /* addrB */ 0x0,
                             /* len */ 1024 * 4, /* ABMode */ XAIE_DISABLE,
                             /* FIFOMode */ XAIE_DISABLE);
  XAieDma_TileBdSetNext(&(ctx->TileDMAInst[27][3]), /* bd */ 1, /* nextbd */ 1);
  XAieDma_TileBdSetPkt(&(ctx->TileDMAInst[27][3]), /* bd */ 1, /* en */ 1,
                       /* type */ 0, /* id */ 1);
  XAieDma_TileBdWrite(&(ctx->TileDMAInst[27][3]), /* bd */ 1);
  XAieDma_TileBdSetLock(&(ctx->TileDMAInst[27][3]), /* bd */ 2,
                        XAIEDMA_TILE_BD_ADDRA, /* lockID */ 1, XAIE_ENABLE,
                        /* release */ 0, XAIE_ENABLE, /* acquire */ 1);
  XAieDma_TileBdSetAdrLenMod(&(ctx->TileDMAInst[27][3]), /* bd */ 2,
                             /* addrA */ 0x1000, /* addrB */ 0x0,
                             /* len */ 1024 * 4, /* ABMode */ XAIE_DISABLE,
                             /* FIFOMode */ XAIE_DISABLE);
  XAieDma_TileBdSetNext(&(ctx->TileDMAInst[27][3]), /* bd */ 2, /* nextbd */ 2);
  XAieDma_TileBdSetPkt(&(ctx->TileDMAInst[27][3]), /* bd */ 2, /* en */ 1,
                       /* type */ 0, /* id */ 1);
  XAieDma_TileBdWrite(&(ctx->TileDMAInst[27][3]), /* bd */ 2);
  XAieDma_TileSetStartBd((&(ctx->TileDMAInst[27][3])), XAIEDMA_TILE_CHNUM_S2MM0,
                         /* bd */ 0);
  XAieDma_TileChControl(&(ctx->TileDMAInst[27][3]), XAIEDMA_TILE_CHNUM_S2MM0,
                        XAIE_RESETDISABLE, XAIE_ENABLE);
  XAieDma_TileSetStartBd((&(ctx->TileDMAInst[27][3])), XAIEDMA_TILE_CHNUM_S2MM1,
                         /* bd */ 1);
  XAieDma_TileChControl(&(ctx->TileDMAInst[27][3]), XAIEDMA_TILE_CHNUM_S2MM1,
                        XAIE_RESETDISABLE, XAIE_ENABLE);
  XAieDma_TileSetStartBd((&(ctx->TileDMAInst[27][3])), XAIEDMA_TILE_CHNUM_MM2S0,
                         /* bd */ 2);
  XAieDma_TileChControl(&(ctx->TileDMAInst[27][3]), XAIEDMA_TILE_CHNUM_MM2S0,
                        XAIE_RESETDISABLE, XAIE_ENABLE);
  XAieDma_Shim ShimDMAInst_27_0;
  XAieDma_ShimInitialize(&(ctx->TileInst[27][0]), &ShimDMAInst_27_0);
  XAieDma_ShimBdClearAll(&ShimDMAInst_27_0);
  XAieDma_ShimChResetAll(&ShimDMAInst_27_0);
  ...
  XAieDma_ShimBdSetAddr(&ShimDMAInst_27_0, /* bd */ 0,
                        HIGH_ADDR((u64)0x20100001000),
                        LOW_ADDR((u64)0x20100001000), /* len */ 1024 * 4);
  XAieDma_ShimBdSetAxi(&ShimDMAInst_27_0, /* bd */ 0, /* smid */ 0,
                       /* burstlen */ 4, /* QOS */ 0, /* Cache */ 0,
                       /* secure */ XAIE_ENABLE);
  XAieDma_ShimBdSetNext(&ShimDMAInst_27_0, /* bd */ 0, /* nextbd */ 1);
  XAieDma_ShimBdSetPkt(&ShimDMAInst_27_0, /* bd */ 0, /* en */ 1, /* type */ 0,
                       /* id */ 4);
  XAieDma_ShimBdWrite(&ShimDMAInst_27_0, /* bd */ 0);
  XAieDma_ShimBdSetLock(&ShimDMAInst_27_0, /* bd */ 1, /* lockID */ 0,
                        XAIE_ENABLE, /* release */ 0, XAIE_ENABLE,
                        /* acquire */ 1);
  XAieDma_ShimBdSetAddr(&ShimDMAInst_27_0, /* bd */ 1,
                        HIGH_ADDR((u64)0x20100000000),
                        LOW_ADDR((u64)0x20100000000), /* len */ 1024 * 4);
  XAieDma_ShimBdSetAxi(&ShimDMAInst_27_0, /* bd */ 1, /* smid */ 0,
                       /* burstlen */ 4, /* QOS */ 0, /* Cache */ 0,
                       /* secure */ XAIE_ENABLE);
  XAieDma_ShimBdSetNext(&ShimDMAInst_27_0, /* bd */ 1, /* nextbd */ 0);
  XAieDma_ShimBdSetPkt(&ShimDMAInst_27_0, /* bd */ 1, /* en */ 1, /* type */ 0,
                       /* id */ 5);
  XAieDma_ShimBdWrite(&ShimDMAInst_27_0, /* bd */ 1);
  ...
  XAieDma_ShimBdSetAddr(&ShimDMAInst_26_0, /* bd */ 0,
                        HIGH_ADDR((u64)0x20100006000),
                        LOW_ADDR((u64)0x20100006000), /* len */ 1025 * 4);
  XAieDma_ShimBdSetAxi(&ShimDMAInst_26_0, /* bd */ 0, /* smid */ 0,
                       /* burstlen */ 4, /* QOS */ 0, /* Cache */ 0,
                       /* secure */ XAIE_ENABLE);
  XAieDma_ShimBdSetNext(&ShimDMAInst_26_0, /* bd */ 0, /* nextbd */ 1);
  XAieDma_ShimBdSetPkt(&ShimDMAInst_26_0, /* bd */ 0, /* en */ 1, /* type */ 0,
                       /* id */ 0);
  XAieDma_ShimBdWrite(&ShimDMAInst_26_0, /* bd */ 0);
  XAieDma_ShimBdSetAddr(&ShimDMAInst_26_0, /* bd */ 1,
                        HIGH_ADDR((u64)0x20100008000),
                        LOW_ADDR((u64)0x20100008000), /* len */ 1025 * 4);
  XAieDma_ShimBdSetAxi(&ShimDMAInst_26_0, /* bd */ 1, /* smid */ 0,
                       /* burstlen */ 4, /* QOS */ 0, /* Cache */ 0,
                       /* secure */ XAIE_ENABLE);
  XAieDma_ShimBdSetNext(&ShimDMAInst_26_0, /* bd */ 1, /* nextbd */ 0);
  ...
  XAieDma_ShimBdSetLock(&ShimDMAInst_26_0, /* bd */ 2, /* lockID */ 1,
                        XAIE_ENABLE, /* release */ 0, XAIE_ENABLE,
                        /* acquire */ 1);
  XAieDma_ShimBdSetAddr(&ShimDMAInst_26_0, /* bd */ 2,
                        HIGH_ADDR((u64)0x20100004000),
                        LOW_ADDR((u64)0x20100004000), /* len */ 1024 * 4);
  XAieDma_ShimBdSetAxi(&ShimDMAInst_26_0, /* bd */ 2, /* smid */ 0,
                       /* burstlen */ 4, /* QOS */ 0, /* Cache */ 0,
                       /* secure */ XAIE_ENABLE);
  XAieDma_ShimBdSetNext(&ShimDMAInst_26_0, /* bd */ 2, /* nextbd */ 3);
  ...
  XAieDma_ShimBdSetLock(&ShimDMAInst_26_0, /* bd */ 3, /* lockID */ 2,
                        XAIE_ENABLE, /* release */ 0, XAIE_ENABLE,
                        /* acquire */ 1);
  XAieDma_ShimBdSetAddr(&ShimDMAInst_26_0, /* bd */ 3,
                        HIGH_ADDR((u64)0x20100002000),
                        LOW_ADDR((u64)0x20100002000), /* len */ 1024 * 4);
  XAieDma_ShimBdSetAxi(&ShimDMAInst_26_0, /* bd */ 3, /* smid */ 0,
                       /* burstlen */ 4, /* QOS */ 0, /* Cache */ 0,
                       /* secure */ XAIE_ENABLE);
  XAieDma_ShimBdSetNext(&ShimDMAInst_26_0, /* bd */ 3, /* nextbd */ 2);
  XAieDma_ShimBdSetPkt(&ShimDMAInst_26_0, /* bd */ 3, /* en */ 1, /* type */ 0,
                       /* id */ 1);
  XAieDma_ShimBdWrite(&ShimDMAInst_26_0, /* bd */ 3);
  XAieDma_ShimBdSetLock(&ShimDMAInst_26_0, /* bd */ 4, /* lockID */ 3,
                        XAIE_ENABLE, /* release */ 0, XAIE_ENABLE,
                        /* acquire */ 1);
  XAieDma_ShimBdSetAddr(&ShimDMAInst_26_0, /* bd */ 4,
                        HIGH_ADDR((u64)0x20100005000),
                        LOW_ADDR((u64)0x20100005000), /* len */ 1024 * 4);
  XAieDma_ShimBdSetAxi(&ShimDMAInst_26_0, /* bd */ 4, /* smid */ 0,
                       /* burstlen */ 4, /* QOS */ 0, /* Cache */ 0,
                       /* secure */ XAIE_ENABLE);
  XAieDma_ShimBdSetNext(&ShimDMAInst_26_0, /* bd */ 4, /* nextbd */ 5);
  XAieDma_ShimBdSetPkt(&ShimDMAInst_26_0, /* bd */ 4, /* en */ 1, /* type */ 0,
                       /* id */ 4);
  XAieDma_ShimBdWrite(&ShimDMAInst_26_0, /* bd */ 4);
  XAieDma_ShimBdSetLock(&ShimDMAInst_26_0, /* bd */ 5, /* lockID */ 0,
                        XAIE_ENABLE, /* release */ 0, XAIE_ENABLE,
                        /* acquire */ 1);
  XAieDma_ShimBdSetAddr(&ShimDMAInst_26_0, /* bd */ 5,
                        HIGH_ADDR((u64)0x20100003000),
                        LOW_ADDR((u64)0x20100003000), /* len */ 1024 * 4);
  XAieDma_ShimBdSetAxi(&ShimDMAInst_26_0, /* bd */ 5, /* smid */ 0,
                       /* burstlen */ 4, /* QOS */ 0, /* Cache */ 0,
                       /* secure */ XAIE_ENABLE);
  XAieDma_ShimBdSetNext(&ShimDMAInst_26_0, /* bd */ 5, /* nextbd */ 4);
  XAieDma_ShimBdSetPkt(&ShimDMAInst_26_0, /* bd */ 5, /* en */ 1, /* type */ 0,
                       /* id */ 5);
  XAieDma_ShimBdWrite(&ShimDMAInst_26_0, /* bd */ 5);
...
void mlir_aie_configure_switchboxes(aie_libxaie_ctx_t *ctx) {
  int x, y;
  // Core Stream Switch column 27 row 0
  x = 27;
  y = 0;
  ...
  XAieTile_StrmConfigSlvSlot(
      ...
      XAIETILE_STRSW_SLVSLOT_CFG(
          &(ctx->TileInst[x][y]),
          (XAIETILE_STRSW_SPORT_SOUTH(&(ctx->TileInst[x][y]), 3)), 0 /*slot*/,
          0x4 /*ID value*/, 0x1E /*mask*/, XAIE_ENABLE, 1 /*msel*/,
          0 /*arbiter*/));
  ...
  XAieTile_StrmConfigSlvSlot(
      ...
      XAIETILE_STRSW_SLVSLOT_CFG(
          &(ctx->TileInst[x][y]),
          (XAIETILE_STRSW_SPORT_SOUTH(&(ctx->TileInst[x][y]), 7)), 0 /*slot*/,
          0x4 /*ID value*/, 0x1E /*mask*/, XAIE_ENABLE, 2 /*msel*/,
          0 /*arbiter*/));
  ...
  XAieTile_StrmConfigSlvSlot(
      ...
      XAIETILE_STRSW_SLVSLOT_CFG(
          ...
          0 /*arbiter*/));
  // Core Stream Switch column 27 row 2
  x = 27;
  y = 2;
  XAieTile_StrmConfigMstr(
      &(ctx->TileInst[x][y]),
  ...
  XAieTile_StrmConfigSlvSlot(
      ...
      XAIETILE_STRSW_SLVSLOT_CFG(
          ...
          0 /*arbiter*/));
  // Core Stream Switch column 27 row 3
  x = 27;
  y = 3;
  XAieTile_StrmConfigMstr(
      &(ctx->TileInst[x][y]),
  ...
  XAieTile_StrmConfigSlvSlot(
      ...
      XAIETILE_STRSW_SLVSLOT_CFG(
          ...
          0x5 /*ID value*/, 0x1F /*mask*/, XAIE_ENABLE, 1 /*msel*/,
          0 /*arbiter*/));
  // Core Stream Switch column 27 row 1
  x = 27;
  y = 1;
  XAieTile_StrmConfigMstr(
      &(ctx->TileInst[x][y]),
  ...
  XAieTile_StrmConfigSlvSlot(
      ...
      XAIETILE_STRSW_SLVSLOT_CFG(
          &(ctx->TileInst[x][y]),
          (XAIETILE_STRSW_SPORT_SOUTH(&(ctx->TileInst[x][y]), 1)), 0 /*slot*/,
          0x0 /*ID value*/, 0x1E /*mask*/, XAIE_ENABLE, 1 /*msel*/,
          0 /*arbiter*/));
  XAieTile_StrmConfigSlv(&(ctx->TileInst[x][y]),
                         XAIETILE_STRSW_SPORT_SOUTH(&(ctx->TileInst[x][y]), 0),
                         XAIE_ENABLE, XAIE_ENABLE);
  XAieTile_StrmConfigSlvSlot(
      ...
      XAIETILE_STRSW_SLVSLOT_CFG(
          &(ctx->TileInst[x][y]),
          (XAIETILE_STRSW_SPORT_SOUTH(&(ctx->TileInst[x][y]), 0)), 0 /*slot*/,
          0x4 /*ID value*/, 0x1E /*mask*/, XAIE_ENABLE, 3 /*msel*/,
          0 /*arbiter*/));
  // ShimMux column 27 row 0
...
int32_t mlir_aie_read_buffer_buf19(aie_libxaie_ctx_t *ctx, int index) {
  int32_t value =
      XAieTile_DmReadWord(&(ctx->TileInst[27][3]), buf19_offset + (index * 4));
  return value;
}
void mlir_aie_write_buffer_buf19(aie_libxaie_ctx_t *ctx, int index,
                                 int32_t value) {
  int32_t int_value = value;
  return XAieTile_DmWriteWord(&(ctx->TileInst[27][3]),
...
int32_t mlir_aie_read_buffer_buf20(aie_libxaie_ctx_t *ctx, int index) {
  int32_t value =
      XAieTile_DmReadWord(&(ctx->TileInst[27][3]), buf20_offset + (index * 4));
  return value;
}
...
int32_t mlir_aie_read_buffer_buf21(aie_libxaie_ctx_t *ctx, int index) {
  int32_t value =
      XAieTile_DmReadWord(&(ctx->TileInst[27][3]), buf21_offset + (index * 4));
  return value;
}
...
const int buf12_offset = 4096;
int32_t mlir_aie_read_buffer_buf12(aie_libxaie_ctx_t *ctx, int index) {
  int32_t value =
      XAieTile_DmReadWord(&(ctx->TileInst[26][3]), buf12_offset + (index * 4));
  return value;
}
void mlir_aie_write_buffer_buf12(aie_libxaie_ctx_t *ctx, int index,
                                 int32_t value) {
  int32_t int_value = value;
  return XAieTile_DmWriteWord(&(ctx->TileInst[26][3]),
                              buf12_offset + (index * 4), int_value);
}
const int buf13_offset = 8192;
int32_t mlir_aie_read_buffer_buf13(aie_libxaie_ctx_t *ctx, int index) {
  int32_t value =
      XAieTile_DmReadWord(&(ctx->TileInst[26][3]), buf13_offset + (index * 4));
  return value;
}
void mlir_aie_write_buffer_buf13(aie_libxaie_ctx_t *ctx, int index,
                                 int32_t value) {
  int32_t int_value = value;
  return XAieTile_DmWriteWord(&(ctx->TileInst[26][3]),
                              buf13_offset + (index * 4), int_value);
}
const int buf14_offset = 12288;
int32_t mlir_aie_read_buffer_buf14(aie_libxaie_ctx_t *ctx, int index) {
  int32_t value =
      XAieTile_DmReadWord(&(ctx->TileInst[26][3]), buf14_offset + (index * 4));
  return value;
}
void mlir_aie_write_buffer_buf14(aie_libxaie_ctx_t *ctx, int index,
                                 int32_t value) {
  int32_t int_value = value;
  return XAieTile_DmWriteWord(&(ctx->TileInst[26][3]),
                              buf14_offset + (index * 4), int_value);
}
const int buf15_offset = 4096;
int32_t mlir_aie_read_buffer_buf15(aie_libxaie_ctx_t *ctx, int index) {
  int32_t value =
      XAieTile_DmReadWord(&(ctx->TileInst[27][2]), buf15_offset + (index * 4));
  return value;
}
void mlir_aie_write_buffer_buf15(aie_libxaie_ctx_t *ctx, int index,
                                 int32_t value) {
  int32_t int_value = value;
  return XAieTile_DmWriteWord(&(ctx->TileInst[27][2]),
...
int32_t mlir_aie_read_buffer_buf16(aie_libxaie_ctx_t *ctx, int index) {
  int32_t value =
      XAieTile_DmReadWord(&(ctx->TileInst[27][2]), buf16_offset + (index * 4));
  return value;
}
...
int32_t mlir_aie_read_buffer_buf17(aie_libxaie_ctx_t *ctx, int index) {
  int32_t value =
      XAieTile_DmReadWord(&(ctx->TileInst[27][2]), buf17_offset + (index * 4));
  return value;
}
...
int32_t mlir_aie_read_buffer_buf18(aie_libxaie_ctx_t *ctx, int index) {
  int32_t value =
      XAieTile_DmReadWord(&(ctx->TileInst[27][2]), buf18_offset + (index * 4));
  return value;
}
...
int32_t mlir_aie_read_buffer_buf9(aie_libxaie_ctx_t *ctx, int index) {
  int32_t value =
      XAieTile_DmReadWord(&(ctx->TileInst[26][2]), buf9_offset + (index * 4));
  return value;
}
...
const int buf10_offset = 12288;
int32_t mlir_aie_read_buffer_buf10(aie_libxaie_ctx_t *ctx, int index) {
  int32_t value =
      XAieTile_DmReadWord(&(ctx->TileInst[26][2]), buf10_offset + (index * 4));
  return value;
}
...
const int buf11_offset = 16384;
int32_t mlir_aie_read_buffer_buf11(aie_libxaie_ctx_t *ctx, int index) {
  int32_t value =
      XAieTile_DmReadWord(&(ctx->TileInst[26][2]), buf11_offset + (index * 4));
  return value;
}
